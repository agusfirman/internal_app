<h3 style="margin-top:80px" class="text-center text-red"> FORM BUKTI SETORAN || BANK MANDIRI</h3>
<hr/> 
<form id="form_cimb" class="form-horizontal" action="cetak/print-out-mandiri.php" method="post" target="_blank" >
	...
	<label class="col-sm-3 control-label">Nama Pemilik Rekening</label>
		<div class="col-sm-3">
		  <select class="form-control input-sm" name="nama"  id="nama" autofocus>
		  	...
		  	$result =$mysqli->query("select id, nama_dokter from dt_dokter where norek_mandiri !='-' order by nama_dokter ASC");
		  	while($data_mandiri = $result->fetch_array()){
		  		echo '<option value="'.$data_mandiri[id].'">'.$data_mandiri[nama_dokter].'</option>';
		  	}
		  	 ?>
		  </select>
		</div>
 ...
 <div class="form-group">
  ...
  <div class="form-group">
	<div class="col-sm-offset-3 col-sm-6">
	  <button type="submit" class="btn btn-primary btn-sm btn-flat"  id="btn_cetak_cimb"><span class="glyphicon glyphicon-print" onclick=""  ></span> 
	  Cetak
	  </button>
